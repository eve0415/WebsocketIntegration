...
class MohistEventListener constructor(instance: MohistPlugin) : Listener {
  ...
  @EventHandler(priority = EventPriority.MONITOR, ignoreCancelled = true)
  fun onPlayerConnectionClose(event: PlayerQuitEvent) {
    manager.send(
      WIEventState.LOG,
      manager.builder().log(
        LogEventType.DISCONNECT, event.player.name,
        event.player.uniqueId, event.player.address.hostName
      ).toJSON()
    )
  }
